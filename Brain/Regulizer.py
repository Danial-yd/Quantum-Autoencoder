...
def qae_combo_loss(
    ...
    *,
    ...
    L_total = lam * L_t + (1.0 - lam) * L_r

    # L2 regularization: l2_reg * ||theta||^2
    if l2_reg != 0.0:
        theta_arr = np.asarray(theta, dtype=float)
        L_total += l2_reg * float(np.sum(theta_arr**2))
...
def train_qae_combo_with_history(
    ...
    fd_eps=1e-3,
    steps_per_epoch=1,
    l2_reg=0.0,        # L2 regularization strength
):
    """
    Trains the QAE combo loss and records loss/fidelity history.
    Uses combo loss:
        L_total = lam * L_trash + (1 - lam) * L_recon + l2_reg * ||theta||^2
    """
    ...
    steps = []
    loss_total_hist = []   # includes L2 term
    loss_trash_hist = []
    loss_recon_hist = []
    fid_trash_hist = []
    fid_recon_hist = []
    ...
    for ep in pbar:
        ...
        for k in range(max(1, steps_per_epoch)):
            g, _ = _fd_grad_combo(
                ...
                ref_angle=ref_angle,
                fd_eps=fd_eps,
                l2_reg=l2_reg,     # pass L2 down into finite-diff loss
            )
        ...
        # total loss WITH L2 regularization (for logging)
        L_total = lam * L_t + (1.0 - lam) * L_r
        if l2_reg != 0.0:
            L_total += l2_reg * float(np.sum(theta**2))

        F_t, F_r = 1.0 - L_t, 1.0 - L_r
    ...
    print(f"Final total loss (with L2): {loss_total_hist[-1]:.6f}")
...
theta, steps, L_tot, L_t, L_r, Ft_hist, Fr_hist = train_qae_combo_with_history(
    ...
    fd_eps=1e-3,
    steps_per_epoch=1,
    l2_reg=0.001,   # <- your requested L2 coefficient
)
...
z_latent = get_bb_bottleneck_latent_Z(X_test_m, theta)

Z_train = get_bb_bottleneck_latent_Z(X_qae_train, theta)

mixed_Z = np.concatenate((z_latent ,X_test_values.reshape(-1,1)), axis=1)
...
plt.plot(rec, prec,
         label=f"PR (AUC = {pr_auc_a:.3f}, FPR ≤ {0.08})",
         color='b', linewidth=2)
